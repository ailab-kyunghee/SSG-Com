...
_base_ = [
    '../datasets/endoscapes/endoscapes_instance_plus_plus.py',
...
custom_imports = dict(imports=orig_imports + ['model.lg', 'evaluator.CocoMetricRGD','hooks.custom_hooks'], allow_failed_imports=False)
...
val_evaluator = dict(
    ...
    ann_file=os.path.join(data_root, 'val/annotation_ds_coco_triplet.json'),
    metric=['bbox'],
...
test_evaluator = dict(
    ...
    ann_file=os.path.join(data_root, 'test/annotation_ds_coco_triplet.json'),
    metric=['bbox'],
    use_pred_boxes_recon=False,
...
default_hooks = dict(
    checkpoint=dict(save_best='endoscapes/bbox_mAP'),
     sampler_seed=dict(type='FixedSeedSamplerHook', seed=42),
)
