...
class LGDetector(BaseDetector):
    ...
    def extract_lg(self, batch_inputs: Tensor, batch_data_samples: SampleList,
        ...
        with torch.no_grad():
            ...
            # print('batch_inputs:', batch_inputs[0].shape)
            # print('batch_data_samples:', batch_data_samples[0].keys())
            # print('batch_data_samples[0] gt_instances keys:', batch_data_samples[0].gt_instances.keys())
            # print('batch_data_samples[0] gt_instances keys labels:', batch_data_samples[0].gt_instances.labels)
            # print('batch_data_samples[0] gt_instances keys bboxes:', batch_data_samples[0].gt_instances.bboxes)
            # print('batch_data_samples[0] ignored_instances keys:', batch_data_samples[0].ignored_instances.keys())
            # print('batch_data_samples[0] ignored_instances keys labels:', batch_data_samples[0].ignored_instances.labels)
            # print('batch_data_samples:', batch_data_samples[0].keys())
            # print('batch_data_samples[0].ignored_instances:', batch_data_samples[0].ignored_instances.keys())
            # print('batch_data_samples[0].gt_instances:', batch_data_samples[0].gt_instances.keys())
        ...
        #=========
        #===== added image to the detector output and detached results =====
        split_imgs = torch.split(batch_inputs, 1, dim=0)
        split_imgs = list(torch.split(batch_inputs, 1, dim=0))
        for i, sample in enumerate(detached_results):
            # With batch dim:
            sample.image = split_imgs[i]  # shape [1, 3, 224, 399]
        #=========
